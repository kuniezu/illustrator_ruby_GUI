...
var LunaMedium = (function () {
    ...
    function apply(port, context, edit) { var before=port.snapshot(), b=clone(context.bundle), a=b.annotation, obs, p, after; if(before.text!==context.snapshot.text||before.note!==context.snapshot.note)fail("stale-source"); port.inspect(b); a.reading=edit.reading;a.enabled=edit.enabled;a.readingConfirmed=edit.readingConfirmed;a.placementMode=edit.placementMode;a.userReview=edit.userReview;a.style=edit.style;a.offset=edit.offset; validate(b); try{obs=a.enabled?port.observe():{status:"complete"};}catch(e){obs={status:"failed",reasons:["observation-failed"]};} after=port.snapshot(); if(after.text!==before.text||after.note!==before.note)fail("source-changed-during-observation"); p=plan(b,before.text,obs); b.revision++; b.annotation.reviewReasons=p.status==="complete"?[]:p.reasons; b.renderStatus=p.status==="complete"?"pending":p.status; var saved=write(before.note,b); port.writeNote(before.note,saved); if(p.status!=="complete")return{bundle:b,plan:p,observation:obs}; try{port.reconcile(b,p);b.renderStatus="complete";}catch(err){b.renderStatus="failed";b.annotation.reviewReasons=["render-failed"]; } port.writeNote(saved,write(saved,b)); return{bundle:b,plan:p,observation:obs}; }
