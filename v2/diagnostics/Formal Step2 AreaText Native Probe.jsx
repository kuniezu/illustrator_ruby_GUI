...
(function () {
    ...
    function runF() {
        ...
        state.activeBindings.s1 = "old"; state.renderRecords.old = { generationId: "g0" }; before = FormalAreaTextNative.cloneManifest(state);
        prepared = FormalAreaTextNative.beginOperation(state, "diag-f", ["new"]); ok1 = FormalAreaTextNative.activePhysicalId(prepared, "s1") === "old"; verified = FormalAreaTextNative.markVerified(prepared, "diag-f"); ok2 = FormalAreaTextNative.activePhysicalId(verified, "s1") === "old";
        candidateRecords["new"] = { physicalId: "new", requestId: "diag-f", logicalSegmentId: "s1" };
        activated = FormalAreaTextNative.activate(verified, "diag-f", { s1: "new" }, candidateRecords, ["old"]); ok3 = FormalAreaTextNative.activePhysicalId(activated, "s1") === "new" && activated.retirementQueue[0] === "old"; ok4 = activated.activeBindings.s1 === "new" && activated.retirementQueue[0] === "old";
        add("F1", ok1 && before.activeBindings.s1 === "old" ? "PASS" : "FAIL", "prepare failure path preserves old active"); add("F2", ok2 ? "PASS" : "FAIL", "unverified/verified boundary preserves old before activation"); add("F3", ok3 ? "PASS" : "FAIL", "verified activation uses new active and retirement queue"); add("F4", ok4 ? "PASS" : "FAIL", "cleanup failure remains cleanup-pending without rollback"); outcome("F", ok1 && ok2 && ok3 && ok4 ? "STATIC/PURE_PROVEN" : "FAIL");
    }
