...
(function () {
    ...
    function runF() { var state = FormalAreaTextNative.createManifest(), before, prepared, verified, activated, ok1, ok2, ok3, ok4; state.activeBindings.s1 = "old"; state.renderRecords.old = { generationId: "g0" }; before = FormalAreaTextNative.cloneManifest(state); prepared = FormalAreaTextNative.beginOperation(state, "diag-f", ["new"]); ok1 = FormalAreaTextNative.activePhysicalId(prepared, "s1") === "old"; verified = FormalAreaTextNative.markVerified(prepared, "diag-f"); ok2 = FormalAreaTextNative.activePhysicalId(verified, "s1") === "old"; activated = FormalAreaTextNative.activate(verified, "diag-f", { s1: "new" }, { new: { physicalId: "new", requestId: "diag-f", logicalSegmentId: "s1" } }, ["old"]); ok3 = FormalAreaTextNative.activePhysicalId(activated, "s1") === "new" && activated.retirementQueue[0] === "old"; ok4 = activated.activeBindings.s1 === "new" && activated.retirementQueue[0] === "old"; add("F1", ok1 && before.activeBindings.s1 === "old" ? "PASS" : "FAIL", "prepare failure path preserves old active"); add("F2", ok2 ? "PASS" : "FAIL", "unverified/verified boundary preserves old before activation"); add("F3", ok3 ? "PASS" : "FAIL", "verified activation uses new active and retirement queue"); add("F4", ok4 ? "PASS" : "FAIL", "cleanup failure remains cleanup-pending without rollback"); outcome("F", ok1 && ok2 && ok3 && ok4 ? "STATIC/PURE_PROVEN" : "FAIL"); }
    ...
    function runHProductionScaffold() {
        var spec, root, body, bt;
        pendingH = 1;
        try {
            spec = makeHSpec(); root = File($.fileName).parent.parent.fsName;
            body = "(function(){var receivedSpec=" + renderSpecLiteral(spec) + ";var root=" + quote(root) + ";var rs=null;var backend=null;var candidate=null;var disposable=null;var result='';try{$.evalFile(File(root+'/formal-step2/area-text-render-spec.js'));$.evalFile(File(root+'/formal-step2/area-text-native.js'));$.evalFile(File(root+'/formal-step2/area-text-native-backend.jsx'));rs=FormalAreaTextRenderSpec.validate(receivedSpec);if(!rs.ok)throw Error('received-spec-invalid:'+rs.reason);var backendSpec=FormalAreaTextRenderSpec.backendSpec(receivedSpec);disposable=app.documents.add();backend=FormalAreaTextNativeBackend(disposable,disposable.layers[0]);candidate=backend.prepareCandidate(backendSpec);rs=backend.verifyCandidate(candidate,backendSpec);if(!rs.ok&&rs.retryable!==false)rs=backend.tryTracking(candidate,backendSpec);if(!rs.ok)throw Error('candidate-unverified:'+rs.reason);result='schema='+receivedSpec.schema+',rendererMode='+receivedSpec.rendererMode+',rendererVersion='+receivedSpec.rendererVersion+',geometryVersion='+receivedSpec.geometryVersion+',requestId='+receivedSpec.requestId+',sourceFrameId='+receivedSpec.sourceFrameId+',annotationId='+receivedSpec.annotationId+',logicalSegmentId='+receivedSpec.logicalSegmentId+',generationId='+receivedSpec.generationId+',physicalId='+receivedSpec.physicalId+',reading='+receivedSpec.reading+',singleCharacter='+receivedSpec.singleCharacter+',finalGeometry='+receivedSpec.finalLeft+':'+receivedSpec.finalTop+':'+receivedSpec.finalWidth+':'+receivedSpec.finalHeight+',verify='+rs.reason;}catch(e){result='CAPABILITY_UNAVAILABLE:'+String(e.message||e);}finally{try{if(backend&&candidate)backend.disposeCandidate(candidate);}catch(e1){}try{if(disposable)disposable.close(SaveOptions.DONOTSAVECHANGES);}catch(e2){}}result;})();";
            bt = new BridgeTalk(); bt.target = BridgeTalk.getSpecifier("illustrator"); bt.body = body;
            bt.onResult = function (result) { completeH("PASS", "production-scaffold receiver=" + s(result.body)); };
            bt.onError = function (error) { completeH("CAPABILITY_UNAVAILABLE", "production-scaffold receiver=" + s(error.body || error)); };
            bt.onTimeout = function () { completeH("CAPABILITY_UNAVAILABLE", "reason=callback-timeout"); };
            if (!bt.send(30)) completeH("CAPABILITY_UNAVAILABLE", "send=false"); else add("H", "PENDING", "complete report waits for production scaffold callback");
        } catch (e) { completeH("CAPABILITY_UNAVAILABLE", "production-scaffold=" + s(e.message || e)); }
    }
    function runHRenderSpec() {
        var backendSpec, body, bt;
        pendingH = 1;
        try {
            backendSpec = makeHSpec();
            if (typeof BridgeTalk === "undefined") { completeH("CAPABILITY_UNAVAILABLE", "BridgeTalk-unavailable"); return; }
            body = "(function(){var p='" + backendSpec.physicalId + "|" + backendSpec.reading + "|" + backendSpec.appearance.fontSize + "|" + backendSpec.left + "|" + backendSpec.top + "|" + backendSpec.width + "|" + backendSpec.height + "'.split('|'),d=null,l=null,x=null,f=null,r=null,o='';try{d=app.documents.add();l=d.layers[0];x=l.pathItems.rectangle(Number(p[4]),Number(p[3]),Number(p[5]),Number(p[6]));x.filled=false;x.stroked=false;f=d.textFrames.areaText(x);f.contents=p[1];r=f.textRange;r.characterAttributes.size=Number(p[2]);r.characterAttributes.horizontalScale=100;r.characterAttributes.verticalScale=100;r.paragraphAttributes.justification=Justification.FULLJUSTIFY;o='schema=\'formal-area-text-render-spec:v1\',physicalId='+p[0]+',reading='+f.contents+',kind='+f.kind+',geometry='+f.left+':'+f.top+':'+f.width+':'+f.height+',lines='+r.lines.length;}catch(e){o='FAIL:'+String(e.message||e);}finally{try{if(f)f.remove();}catch(e1){}try{if(x&&x.parent)x.remove();}catch(e2){}try{if(d)d.close(SaveOptions.DONOTSAVECHANGES);}catch(e3){}}o;})();";
            bt = new BridgeTalk(); bt.target = BridgeTalk.getSpecifier("illustrator"); bt.body = body;
            bt.onResult = function (result) { completeH("PASS", "RenderSpec/backendSpec receiver=" + s(result.body)); };
            bt.onError = function (error) { completeH("CAPABILITY_UNAVAILABLE", "RenderSpec receiver=" + s(error.body || error)); };
            bt.onTimeout = function () { completeH("CAPABILITY_UNAVAILABLE", "reason=callback-timeout"); };
            if (!bt.send(30)) completeH("CAPABILITY_UNAVAILABLE", "send=false"); else add("H", "PENDING", "RenderSpec sent; final report waits for callback");
        } catch (e) { completeH("CAPABILITY_UNAVAILABLE", "RenderSpec=" + s(e.message || e)); }
    }
    ...
    runGVersionCheck();
    function runE() { var entry = create("E", 80, 80, 70, 50, "あいうえおかきくけこ", 20), result, stable; if (!entry) { outcome("E", "FAIL"); return; } policy("E", entry, false); result = FormalAreaTextNativeDiagnostic.runTracking(TRACKING, function (trackingValue) { var range = entry.frame.textRange, observation; range.characterAttributes.tracking = trackingValue; observation = observe("E" + trackingValue, entry); if (!observation || !observation.stable) return { ok: false, retryable: false, reason: "observation-unstable" }; if (entry.frame.kind !== TextType.AREATEXT || entry.frame.orientation !== TextOrientation.HORIZONTAL || entry.frame.previousFrame || entry.frame.nextFrame) return { ok: false, retryable: false, reason: "identity-or-threading-mismatch" }; stable = fit("E" + trackingValue, entry, "あいうえおかきくけこ"); return { ok: stable, retryable: true, reason: stable ? "verified-fit" : "coverage-nonfit" }; }); add("E", result.ok ? "PASS" : "MANUAL_REQUIRED", "firstVerified=" + (result.ok ? result.tracking : "none") + ",trials=" + result.trials.length + ",bounded=" + TRACKING.join("/")); outcome("E", result.ok ? "PASS" : "MANUAL_REQUIRED"); cleanup(entry); }
    function runHProductionScaffold() { var spec, root, body, bt; pendingH = 1; try { spec = makeHSpec(); root = quote(File($.fileName).parent.parent.fsName); body = FormalAreaTextNativeDiagnostic.buildReceiverBody(renderSpecLiteral(spec), root); if (typeof BridgeTalk === "undefined") { completeH("CAPABILITY_UNAVAILABLE", "BridgeTalk-unavailable"); return; } bt = new BridgeTalk(); bt.target = BridgeTalk.getSpecifier("illustrator"); bt.body = body; bt.onResult = function (result) { completeH("PASS", "complete-RenderSpec-E2E receiver=" + s(result.body)); }; bt.onError = function (error) { completeH("CAPABILITY_UNAVAILABLE", "complete-RenderSpec-E2E receiver=" + s(error.body || error)); }; bt.onTimeout = function () { completeH("CAPABILITY_UNAVAILABLE", "reason=callback-timeout"); }; if (!bt.send(30)) completeH("CAPABILITY_UNAVAILABLE", "send=false"); else add("H", "PENDING", "complete report waits for generated receiver callback"); } catch (e) { completeH("CAPABILITY_UNAVAILABLE", "generated-RenderSpec-E2E=" + s(e.message || e)); } }
