...
(function () {
    ...
    function cleanup(entry) {
        ...
        try { if (entry.path && entry.path.parent) { entry.path.remove(); pathRemoved = true; } } catch (e2) { add(entry.caseId, "INFO", "path-after-frame-invalidation=" + s(e2.message || e2)); }
    ...
    function runE() {
        ...
        result = FormalAreaTextNativeDiagnostic.runTracking(TRACKING, function (trackingValue) {
            ...
            if (entry.frame.kind !== TextType.AREATEXT || entry.frame.orientation !== TextOrientation.HORIZONTAL || entry.frame.previousFrame || entry.frame.nextFrame) { add("E", "TRIAL", "tracking=" + trackingValue + ",fit=false,retryable=false,stopReason=identity-or-threading-mismatch"); return { ok: false, retryable: false, reason: "identity-or-threading-mismatch" }; }
            ...
            return { ok: stable, retryable: true, reason: stable ? "verified-fit" : "coverage-nonfit" };
        });
