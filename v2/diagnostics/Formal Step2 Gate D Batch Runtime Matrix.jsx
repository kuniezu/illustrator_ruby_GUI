...
(function () {
    ...
    function fitFixture(item, target) { var a = FormalStep2Adapter(app.activeDocument, item), i, o; for (i = 0; i < 6; i++) { o = a.observe(); if (o.status === "complete" && o.lines.length === target) return o; if (!o.lines || o.lines.length > target) item.width *= 1.35; else item.width *= .7; app.redraw(); } return a.observe(); }
    ...
    try {
        ...
        run("D1", function () { var source = fixture("D", "一二三四", 100), a = FormalStep2Adapter(app.activeDocument, source), o = fitFixture(source, 3), b = bundle("一二三四", "いちにさんよん", []); emit("D1", o.status === "complete" && o.lines.length === 3 ? "PASS" : "MANUAL_REQUIRED", "geometry", details(o) + ";fontSize=70"); if (o.status === "complete" && o.lines.length === 3) { var hints = [{baseBoundaryAfter:o.lines[0].end,readingBoundaryAfter:2,baseText:"一二三四",reading:"いちにさんよん"},{baseBoundaryAfter:o.lines[1].end,readingBoundaryAfter:4,baseText:"一二三四",reading:"いちにさんよん"}]; emit("D2", hints[0].readingBoundaryAfter < hints[1].readingBoundaryAfter ? "PASS" : "FAIL", "hint-fixture", "readingBoundaries=2,4"); b.splitHints = hints; planAndRender("D3", a, b, o, "いちにさんよん", hints); } });
        run("E1", function () { var source = fixture("E1", "A一B張C羅123", 180), a = FormalStep2Adapter(app.activeDocument, source), o = a.observe(), b = bundle("A一B張C羅123", "えーいちびーちょうしーら123", []); if (o.status === "complete" && o.lines.length) { var d = FormalSegments.plan("A一B張C羅123", "えーいちびーちょうしーら123", o.lines, [], 0, 0); emit("E1", d.status === "complete" ? "PASS" : "MANUAL_REQUIRED", "proportional-mixed", details(o) + ";font=" + String(o.fontName || "available-via-DOM")); } else emit("E1", "MANUAL_REQUIRED", "proportional-mixed", details(o)); });
