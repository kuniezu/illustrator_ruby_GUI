...
(function () {
    ...
    function fixture(caseId, text, width) {
        ...
        item.contents = text; item.note = "gate-d-fixture:" + caseId; fixtures.push({item: item, path: path}); return item;
    }
    ...
    function cleanup() {
        ...
        for (i = fixtures.length - 1; i >= 0; i--) try { if (fixtures[i].item && fixtures[i].item.parent) fixtures[i].item.remove(); if (fixtures[i].path && fixtures[i].path.parent) fixtures[i].path.remove(); } catch (e2) { emit("CLEANUP", "FAIL", "fixture", e2.message || e2); }
    }
