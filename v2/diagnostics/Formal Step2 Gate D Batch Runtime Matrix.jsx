...
(function () {
    ...
    try {
        ...
        run("C1", function () { var source = fitFixture(fixture("C", "一張羅", 120), 2), a = FormalStep2Adapter(app.activeDocument, source), o = a.observe(), h = [{baseBoundaryAfter: 2, readingBoundaryAfter: 5, baseText: "一張羅", reading: "いっちょうら"}], b = bundle("一張羅", "いっちょうら", h), twoLineDecision, one, two, actualReflow = false; if (o.status === "complete" && o.lines.length === 2) twoLineDecision = planAndRender("C1", a, b, o, "いっちょうら", h); else emit("C1", "MANUAL_REQUIRED", "initial-two-line", details(o)); source.width = 1000; app.redraw(); one = a.observe(); actualReflow = one.status === "complete" && one.lines.length === 1; emit("C2", actualReflow ? "PASS" : "MANUAL_REQUIRED", "actual-reflow", details(one)); source.width = 120; app.redraw(); two = a.observe();
            /* C3 deliberately uses renderer-only desired states. It must remain
               meaningful when Illustrator cannot prove the real 2->1 reflow. */
            if (twoLineDecision && twoLineDecision.status === "complete") {
                a.reconcile(b, {status: "complete", segments: [twoLineDecision.segments[0]]});
                var rendererOne = managedCount(a, b) === 1;
                a.reconcile(b, twoLineDecision);
                var rendererTwo = managedCount(a, b) === 2;
                emit("C3", rendererOne && rendererTwo ? "PASS" : "FAIL", "renderer-lifecycle", "desired=2->1->2;managed=" + managedCount(a, b) + ";actualReflow=" + (actualReflow ? "observed" : "unobserved"));
            } else emit("C3", "FAIL", "renderer-lifecycle", "initial-two-line-plan-unavailable");
        });
        run("D1", function () { var source = fitFixture(fixture("D", "一二三四五六", 120), 3), a = FormalStep2Adapter(app.activeDocument, source), o = a.observe(), b = bundle("一二三四五六", "いちにさんよんごろく", []); emit("D1", o.status === "complete" && o.lines.length === 3 ? "PASS" : "MANUAL_REQUIRED", "geometry", details(o) + ";fontSize=70;fixture=6glyphs"); if (o.status === "complete" && o.lines.length === 3) { var hints = [{baseBoundaryAfter:o.lines[0].end,readingBoundaryAfter:4,baseText:"一二三四五六",reading:"いちにさんよんごろく"},{baseBoundaryAfter:o.lines[1].end,readingBoundaryAfter:8,baseText:"一二三四五六",reading:"いちにさんよんごろく"}]; emit("D2", hints[0].readingBoundaryAfter < hints[1].readingBoundaryAfter ? "PASS" : "FAIL", "hint-fixture", "readingBoundaries=4,8"); b.splitHints = hints; planAndRender("D3", a, b, o, "いちにさんよんごろく", hints); } });
