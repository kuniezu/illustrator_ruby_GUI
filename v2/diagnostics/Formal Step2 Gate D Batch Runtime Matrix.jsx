...
(function () {
    ...
    function fitFixture(item, target) { var i, o, entry, old, nextWidth, next; for (i = 0; i < 6; i++) { o = FormalStep2Adapter(app.activeDocument, item).observe(); if (o.status === "complete" && o.lines.length === target) return item; nextWidth = item.width * (o.lines && o.lines.length > target ? 1.35 : .7); for (var k = 0; k < fixtures.length; k++) if (fixtures[k].item === item) entry = fixtures[k]; old = item; next = fixture(String(item.note).split(":")[1] || "retry", String(item.contents), nextWidth); next.textRange.characterAttributes.size = 70; if (entry) entry.item = next; try { if (old.parent) old.remove(); } catch (ignore) {} item = next; app.redraw(); } return item; }
