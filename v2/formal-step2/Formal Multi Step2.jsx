        ...
        saveButton.onClick = function () {
            ...
            try {
                ...
                result = FormalMultiPersistenceAdapter.saveRendered(bundle.textSnapshot, cachedNote, bundle, sourceIdentity, FormalMultiRenderer.specifications(bundle), renderSources, {
                    ...
                    failure: function (diagnostics) { var persisted, uncertain = diagnostics && diagnostics.kind === "transport-uncertain", reason = diagnostics && diagnostics.reason ? diagnostics.reason : diagnostics, failureText; if (requestId !== activeSaveRequestToken || requestRevision !== bundle.revision) return; setSavePending(false); if (diagnostics && diagnostics.noteVerified === true) { cachedNote = diagnostics.note; persisted = FormalMultiStore.read(cachedNote); if (persisted) { bundle = persisted; refreshList(); } } if (uncertain) { transportUncertain = true; saveButton.enabled = false; showDiagnostics(reason || "transport result uncertain"); stateText.text = "状態: 結果不確定 / 再保存を停止しました。閉じて再同期・recovery後に再開してください"; return; } showDiagnostics(reason); failureText = reason instanceof Array ? reason.join(" | ") : String(reason); stateText.text = "状態: 保存失敗 / " + failureText; alert("Formal Step 2 保存に失敗しました。\n" + failureText); }
