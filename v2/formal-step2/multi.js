...
var FormalMulti = (function () {
    ...
    function copyAnnotation(a) {
        var h = a.anchor;
        return {annotationId:a.annotationId,sourceFrameId:a.sourceFrameId,anchor:{baseText:h.baseText,startHint:h.startHint,beforeContext:h.beforeContext,afterContext:h.afterContext},reading:a.reading,enabled:a.enabled,placementMode:a.placementMode,reviewReasons:a.reviewReasons.slice(0),readingConfirmed:a.readingConfirmed,style:{sizeRatio:a.style.sizeRatio,gapRatio:a.style.gapRatio},offset:{inlineEm:a.offset.inlineEm,blockEm:a.offset.blockEm},splitHints:(a.splitHints || []).slice(0)};
    }
    function validateAnnotation(bundle, annotation) {
        ...
        for (var i=0;i<annotation.splitHints.length;i++) {
            ...
            if (!number(h.baseBoundaryAfter)||!number(h.readingBoundaryAfter)||h.baseBoundaryAfter<=0||h.readingBoundaryAfter<=0||typeof h.baseText!=="string"||typeof h.reading!=="string") fail("invalid-multi-split-hint");
        }
    }
    ...
    function add(bundle, annotation) { var next=clone(bundle); var a=copyAnnotation(annotation); a.splitHints=(annotation.splitHints||[]).slice(0); next.annotations.push(a); return validate(next); }
    function update(bundle, annotationId, edit) { var next=clone(bundle), found=false; for(var i=0;i<next.annotations.length;i++) if(next.annotations[i].annotationId===annotationId){for(var key in edit) if(edit.hasOwnProperty(key)&&key!=="annotationId"&&key!=="sourceFrameId") next.annotations[i][key]=edit[key]; found=true; break;} if(!found) fail("multi-annotation-missing"); return validate(next); }
