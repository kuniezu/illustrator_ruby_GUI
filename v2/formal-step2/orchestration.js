...
var FormalMultiOrchestration = (function () {
    ...
    function planOne(bundle, annotationId, sourceText, observation) {
        ...
        return {annotationId:annotationId,status:decision.status,sourceStart:resolved.start,sourceEnd:resolved.start+annotation.anchor.baseText.length,decision:decision,reasons:decision.reasons||[],boundaries:decision.boundaries||[],unresolvedBoundaries:decision.unresolvedBoundaries||[],reusableHints:decision.reusableHints||[]};
    }
    function planAll(bundle, sourceText, observation) { var results=[],i,hasFailed=false,hasUnresolved=false; FormalMulti.validate(bundle); for(i=0;i<bundle.annotations.length;i++){results.push(planOne(bundle,bundle.annotations[i].annotationId,sourceText,observation));if(results[i].status==="failed")hasFailed=true;else if(results[i].status!=="complete"&&results[i].status!=="hidden")hasUnresolved=true;} return {status:hasFailed?"failed":(hasUnresolved?"unresolved":"complete"),results:results}; }
