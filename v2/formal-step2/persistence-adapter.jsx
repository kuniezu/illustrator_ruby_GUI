...
var FormalMultiPersistenceAdapter = (function () {
    ...
    function renderedBridgeBody(expectedText, cachedNote, nextNote, renderFailureNote, identity, bundle, specifications, sources, stagePath, requestId) {
        ...
        return "(function(){"+
            ...
            "for(i=0;i<specs.length;i++){spec=specs[i];if(!spec.annotation||!spec.annotation.enabled){plans.push({annotationId:spec.annotationId,decision:{status:\"complete\",segments:[]}});continue;}one={textSnapshot:expected,revision:"+String(bundle.revision)+",annotations:[spec.annotation]};result=FormalMultiOrchestration.planOne(one,spec.annotationId,expected,observation);if(result.status!==\"complete\"){renderError=\"render-unresolved\";plans.push({annotationId:spec.annotationId,decision:{status:\"complete\",segments:[]}});continue;}plans.push({annotationId:spec.annotationId,decision:result.decision});}try{renderAdapter.renderAndStoreTransaction(\""+encoded(bundle.sourceFrameId)+"\",plans,function(){commitStarted=true;frame.note=renderError?renderFailure:next;if(String(frame.note)!==String(renderError?renderFailure:next))fail(\"store-readback-mismatch\");});}catch(error){if(commitStarted)fail(error.message||error);renderError=error;}if(renderError){return \"B-render-persist:render-failed\";}return \"B-render-persist:success\";}());";
    }
