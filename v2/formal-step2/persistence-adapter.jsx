...
var FormalMultiPersistenceAdapter = (function () {
    ...
    function renderedBridge(expectedText, cachedNote, nextNote, renderFailureNote, identity, bundle, specifications, sources, callbacks, bridgeTalkRef, stagePath, requestId) {
        ...
        function finish(callback,value){if(finished)return;finished=true;removeMessage(bt);callback(value);}
        bt.onResult=function(result){var body=result&&result.body?String(result.body):"",prefix="B-render-persist:success:";if(body.substring(0,prefix.length)===prefix)finish(callbacks.success,{strategy:"B-render-persist",note:decodeURIComponent(body.substring(prefix.length))});else if(body==="B-render-persist:render-failed")finish(callbacks.success,{strategy:"B-render-persist",note:renderFailureNote,renderStatus:"failed"});else if(body==="B-render-persist:persistence-failed")finish(callbacks.failure,"B-render-persist: persistence-failed");else finish(callbacks.failure,"B-render-persist: invalid-result:"+(body||"empty"));};
