...
var FormalMultiPersistenceAdapter = (function () {
    ...
    function renderedBridgeBody(expectedText, cachedNote, nextNote, renderFailureNote, identity, bundle, specifications, sources, stagePath, requestId) {
        ...
        return "(function(){"+
            ...
            "for(i=0;i<specs.length;i++){spec=specs[i];if(!spec.annotation||!spec.annotation.enabled){plans.push({annotationId:spec.annotationId,decision:{status:\"complete\",segments:[]}});continue;}one={textSnapshot:expected,revision:"+String(bundle.revision)+",annotations:[spec.annotation]};result=FormalMultiOrchestration.planOne(one,spec.annotationId,expected,observation);if(result.status!==\"complete\"){renderError=\"render-unresolved\";if(result.outcome===\"hidden-confirmed\")plans.push({annotationId:spec.annotationId,decision:{status:\"complete\",segments:[]}});continue;}plans.push({annotationId:spec.annotationId,decision:result.decision});}try{renderAdapter.renderAndStoreTransaction(\""+encoded(bundle.sourceFrameId)+"\",plans,function(){commitStarted=true;frame.note=renderError?renderFailure:next;if(String(frame.note)!==String(renderError?renderFailure:next))fail(\"store-readback-mismatch\");});}catch(error){persistenceError=error;}if(persistenceError){return \"B-render-persist:persistence-failed\";}if(renderError){return \"B-render-persist:render-failed\";}return \"B-render-persist:success\";}());";
    }
    function renderedBridge(expectedText, cachedNote, nextNote, renderFailureNote, identity, bundle, specifications, sources, callbacks, bridgeTalkRef, stagePath, requestId) {
        ...
        function finish(callback,value){if(finished)return;finished=true;removeMessage(bt);callback(value);}
        bt.onResult=function(result){if(result&&result.body==="B-render-persist:success")finish(callbacks.success,{strategy:"B-render-persist",note:nextNote});else if(result&&result.body==="B-render-persist:render-failed")finish(callbacks.success,{strategy:"B-render-persist",note:renderFailureNote,renderStatus:"failed"});else if(result&&result.body==="B-render-persist:persistence-failed")finish(callbacks.failure,"B-render-persist: persistence-failed");else finish(callbacks.failure,"B-render-persist: invalid-result:"+(result&&result.body?result.body:"empty"));};
