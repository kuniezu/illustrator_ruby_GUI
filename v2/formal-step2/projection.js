...
var FormalMultiProjection = (function () {
    ...
    function id(bundle, occurrence) { var text=bundle.sourceFrameId+":"+occurrence.occurrenceId, hash=0, i; for(i=0;i<text.length;i++) hash=(hash*31+text.charCodeAt(i))%1000000000; return prefix+hash+"-0"; }
    ...
    function project(bundle) {
        var next=FormalMulti.clone(bundle), annotations=[], occurrence, existing, generated={}, i;
        if(bundle.occurrences===undefined) return next;
        for(i=0;i<bundle.occurrences.length;i++) { occurrence=bundle.occurrences[i]; if(!eligible(occurrence)) continue; existing=find(next.annotations,id(bundle,occurrence)); if(existing) { if(existing.anchor.baseText!==occurrence.surface||existing.anchor.startHint!==occurrence.start||existing.reading!==occurrence.reading) { existing.anchor.baseText=occurrence.surface; existing.anchor.startHint=occurrence.start; existing.reading=occurrence.reading; existing.readingConfirmed=true; existing.splitHints=[]; existing.reviewReasons=[]; } annotations.push(existing); generated[existing.annotationId]=true; } else annotations.push(create(bundle,occurrence)); }
        for(i=0;i<bundle.annotations.length;i++) if(bundle.annotations[i].annotationId.indexOf(prefix)!==0) annotations.push(bundle.annotations[i]);
