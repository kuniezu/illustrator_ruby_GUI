...
test('bookkeeping files keep the worker handoff contract across Windows EOL',()=>{const read=file=>fs.readFileSync(file,'utf8').replace(/\r\n/g,'\n');assert.match(read(path.join(root,'v2/formal-step2/04_Work整理結果/01_次これやって.md')),/^- dispatch_id:|\n- dispatch_id:/m);assert.match(read(path.join(root,'v2/formal-step2/04_Work整理結果/02_今これやったよ.md')),/^- dispatch_id:|\n- dispatch_id:/m);});
